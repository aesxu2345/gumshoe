...
var (
	ErrSaltTooShort = errors.New("bcrypt: salt must be 16 bytes")
)
...
func GenerateFromPassword_with_salt(password []byte, cost int, salt []byte) ([]byte, error) {
	if len(password) > 72 {
		return nil, ErrPasswordTooLong
	}
	...
	hash, err := bcrypt(password, cost, encodedSalt)
	if err != nil {
		return nil, err
	}
	p := &hashed{
...
// GenerateFromPassword_return_salt 生成 bcrypt 哈希并返回哈希及原始盐值。
// 返回的盐值为 16 字节的原始字节，可用于后续哈希生成。
func GenerateFromPassword_return_salt(password []byte, cost int) ([]byte, []byte, error) {
	if len(password) > 72 {
	...
	return p.Hash(), unencodedSalt, nil
}
